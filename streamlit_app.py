...
if st.session_state.app_mode == 'menu':
    st.markdown("<h1 style='text-align: center;'>SQM TERMINAL</h1>", unsafe_allow_html=True)
...
# --- BIURO ---
    ...
    with t1:
        ev = st.text_input("Dodaj Event:")
        if st.button("Zapisz") and ev: st.session_state.events_list.append(ev); st.rerun()
...
# --- MAGAZYN ---
    ...
    if auta:
        ...
        with st.sidebar.form("add_form"):
            ...
            if st.form_submit_button("DODAJ RZĄD"):
                z1_t = ", ".join([f"{k}: {v}" for k,v in z1_qty.items()])
                z2_t = ", ".join([f"{k}: {v}" for k,v in z2_qty.items()])
                new_row = pd.DataFrame([{'Event':ev_sel, 'Naczepa':auto_sel, 'Rząd':rz, 'Układ':uk, 'Projekt_1':p1, 'Zawartosc_1':z1_t, 'Projekt_2':p2, 'Zawartosc_2':z2_t, 'Uwagi':uw}])
                st.session_state.cargo_db = pd.concat([st.session_state.cargo_db, new_row], ignore_index=True)
                sync_db(); st.rerun()
...
# --- TARGI ---
    ...
    if auta:
        ...
        t1, t2, t3 = st.tabs(["Widok 3D", "Manifest LIFO", "Podsumowanie Sprzętu"])
