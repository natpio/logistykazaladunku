...
# ==========================================
    ...
    tab1, tab2, tab3 = st.tabs(["🎪 Słownik Eventów", "🚛 Baza Floty (Przypisanie)", "📂 Baza Projektów i Kolorów"])

    with tab1:
        st.subheader("Zarządzanie Eventami")
    ...
    with tab2:
        st.subheader("Przypisanie Aut do Eventów")
        with st.form("dodaj_auto", clear_on_submit=True):
            colA, colB = st.columns(2)
            a_event = colA.selectbox("Przypisz do Eventu:", st.session_state.events_list)
            a_nazwa = colB.text_input("Rejestracja / Opis naczepy (np. PO 1234A):")
            
            if st.form_submit_button("💾 Zapisz Auto w Bazie"):
                if a_nazwa:
                    nowe_auto = pd.DataFrame([{"Event": a_event, "Auto": a_nazwa}])
                    st.session_state.fleet_db = pd.concat([st.session_state.fleet_db, nowe_auto], ignore_index=True)
                    st.rerun()
                
        st.markdown("### Aktualna Flota (Zależna od Eventu)")
        edited_fleet = st.data_editor(st.session_state.fleet_db, num_rows="dynamic", use_container_width=True)
        if st.button("Aktualizuj Tabelę Floty"):
            st.session_state.fleet_db = edited_fleet
            st.success("Baza aut zaktualizowana!")
            st.rerun()
...
# ==========================================
    ...
    with st.sidebar:
        ...
        # --- DYNAMICZNE FILTROWANIE FLOTY ---
        df_auta_eventu = st.session_state.fleet_db[st.session_state.fleet_db['Event'] == wybrany_event]
        lista_aut = df_auta_eventu['Auto'].tolist()
        
        if not lista_aut:
            st.warning("⚠️ Logistyk nie przypisał żadnego auta do tego eventu.")
            wybrana_naczepa = None
        else:
            wybrana_naczepa = st.selectbox("Wybierz Auto:", lista_aut)
    ...
    # Ekran główny wyświetla się tylko, gdy jest wybrane auto
    if wybrana_naczepa:
        ...
        st.title("📦 ZAŁADUNEK")
        st.info("Wybierz Event i przypisane do niego Auto z panelu po lewej stronie, aby rozpocząć ładowanie.")
...
# ==========================================
    ...
    with st.sidebar:
        ...
        if not lista_aut:
            st.warning("⚠️ Brak przypisanych aut dla tego eventu.")
            wybrana_naczepa = None
        else:
            wybrana_naczepa = st.selectbox("Które auto rozładowujesz?:", lista_aut)
    ...
    if wybrana_naczepa:
        ...
        with tab_podsumowanie:
            if not df_current_auto.empty:
                ...
                if not df_agregacja.empty:
                    st.dataframe(
                        df_agregacja, use_container_width=True, hide_index=True,
                        column_config={
        ...
        st.title("📥 ROZŁADUNEK")
        st.info("Wybierz Event z menu po lewej, aby zobaczyć dostępne auta.")
